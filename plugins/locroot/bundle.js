function resolveAssetBases() {
  if (!globalThis.__ENTRY_URL__) {
    throw new Error('Locroot plugin entry URL unavailable. Please reinstall the plugin.');
  }
...
function ensureLocaleHost(context) {
  ...
  const api = electron?.locroot ?? electron?.localeForge;
  if (!api) {
    throw new Error('Locroot host API is unavailable in this build.');
  }
  return api;
}
      ...
      ? host.onTranslationProgress((payload) => {
          listeners.forEach((listener) => {
            try {
              ...
              console.error('[Locroot plugin] Progress listener failed', error);
            }
          });
        })
      : () => {};
...
export async function mount(container, context = {}) {
  ...
  const bridgeKey = `__locrootBridge_${crypto.randomUUID ? crypto.randomUUID() : Date.now()}`;
  ...
  try {
    ...
    console.error('[Locroot plugin] Failed to load UI', error);
    container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;background:#0f172a;color:#f8fafc;font-family:Inter,system-ui,sans-serif;border-radius:16px;padding:24px;text-align:center;">
      <div>
        <p style="font-size:16px;font-weight:600;margin-bottom:8px;">Unable to load Locroot</p>
